	...
	for (i = 0; i < 8; i++) {
		for (j = 0; j < 8; j++) {
			...
			if ((*(array+i*8+j) == 6 && turn) || (*(array+i*8+j) == 12 && !turn)) {
				...
				if (j == 4) {
					if (i == 7 && turn) {
						_Bool is_cas = (_Bool) 1;

						for (k = 0; k < hi_sz-1; k++) {
							if (*(history+k*64+56+4) != 6) {
								is_cas = !is_cas;
								break;
							}
						}
...
void get_moves(unsigned char* array, unsigned char* moves, unsigned char* history, int hi_sz, _Bool turn) {
	unsigned char moves_[255][5], moves_opp[255][5], i, j, mlen = 0;
	_Bool is_move;
